    ...
    void clear(TFT_eSPI *canvans)
    {
        ...
        for (size_t i = 0; i <= m.tick; i++, p += strlen(p) + 1)
        {
            ...
            y_tick_value.push_back(
                text().value(p).origin(right).vorigin(vcenter).color(_backgroud).content_width(&w, canvans));
        ...
        if (_note.size())
            draw_tick(_note, x_tick + x_skip_tick - x_tick_addition, width, [&](size_t i, pix_t x)
                      {
            auto p0 = origin(x, 0);
            auto p1 = origin(x, _tick);
            line(p0, p1).color(_backgroud).draw(canvans);
        ...
        if (_value.size())
            draw_tick(_value, m.tick, height, [&](size_t i, pix_t y)
                      {
            ...
            //·绘制刻度线和刻度值
            line(p0, p1).color(_backgroud).draw(canvans);
            y_tick_value[i].xy(p1).backgroud(_backgroud).draw(canvans); });
        ...
        for (size_t i = 0; i < _value.size(); i++)
        {
            ...
            for (int j = 0; j < cur.size() - 1; j++)
            {
                ...
                line(pa, pb).color(_backgroud).draw(canvans);
                if (j == 0)
                {
            ...
            if (show_circle)
            {
                ...
                {
                    ellipse(p, 5, 5).color(_backgroud).fill(white).origin(center).vorigin(vcenter).draw(canvans);
                }
            }
        }
